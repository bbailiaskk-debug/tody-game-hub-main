...
import { scanTags } from "./shared";

export function lintHtmlLike(document: vscode.TextDocument): Issue[] {
  if (!document || document.isClosed) return [];
  const text = document.getText();
  const tags = scanTags(text, document);
...
function findMeta(
  ...
  attrName: string,
  attrValue: string,
): ReturnType<typeof scanTags>[number] | undefined {
  return tags.find(
    (tag) =>
...
function firstLine(document: vscode.TextDocument): vscode.Range {
  return new vscode.Range(new vscode.Position(0, 0), new vscode.Position(0, 0));
}

function extractChildrenRaw(
  document: vscode.TextDocument,
  tag: ReturnType<typeof scanTags>[number],
): string | null {
...
function inRange(position: vscode.Position, startOffset: number, endOffset: number): boolean {
  if (startOffset < 0 || endOffset < 0) return false;
  const start = positionToOffset(position);
  return start > startOffset && start < endOffset;
}

function positionToOffset(position: vscode.Position): number {
